...
xml_dir = "../../../Downloads/0130/wine labels_voc_dataset/"

class Voc_to_yolo_convter() :
    def __init__(self, xml_paths):
        self.xml_path_list = glob.glob(os.path.join(xml_paths,"*.02.voc_to_yolo"))
    ...
    def get_voc_to_yolo(self):
        for xml_path in self.xml_path_list :
            ...
            for object_meta in object_metas:
                ...
                os.makedirs(f"./wine_dataset/{mode}/labels", exist_ok=True)

                # label
                label = label_dict[box_label]

                # txt save
                with open(f"./wine_dataset/{mode}/labels/{image_name_temp}", "a") as f:
