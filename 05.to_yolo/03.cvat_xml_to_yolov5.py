...
def find_xml_file(xml_folder_path) :
    ...
    for (path, dir, files) in os.walk(xml_folder_path) :
        for filename in files :
            ...
            # ext -> .02.voc_to_yolo
            if ext == ".02.voc_to_yolo" :
                root = os.path.join(path, filename)
                # ./xml_data/test.02.voc_to_yolo
                all_root.append(root)
            else :
                print("no 02.voc_to_yolo files.....")
                break
...
# test = glob.glob(os.path.join(xml_folder_path, "*.02.voc_to_yolo"))
