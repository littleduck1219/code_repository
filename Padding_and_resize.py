...
root_path = './code_snippet/dataset_filtering_balanced'
all_data_path = glob.glob(os.path.join(root_path, '*', '*', '*.png'))
for path in all_data_path :
    save_path = path.replace('./dataset_filtering_balanced', 'padding_and_resize224')
    p = save_path.split('/')
    print(p, p[0], p[1], p[2], p[3])
    exit()
    save_folder = os.path.join(p[0], p[1], p[2], p[3])
